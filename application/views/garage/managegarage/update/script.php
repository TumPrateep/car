    ...
    if (data.message == 200) {
        ...
        for (var j = 0; j < 3; j++) {
            if (Service.charAt(j) == "1" && j == 0) {
                $('#change_spare').each(function() {
                    this.checked = true;
                });
            } else if (Service.charAt(j) == "1" && j == 1) {
                $('#change_tire').each(function() {
                    this.checked = true;
                });
            } else if (Service.charAt(j) == "1" && j == 2) {
                $('#change_lubricator').each(function() {
                    this.checked = true;
                });
            }
        }
    ...
    $.post(base_url + "service/Locationforregister/getProvince", {},
        function(data) {
            ...
            $.each(province, function(index, value) {
                provinceDropdownGarage.append('<option value="' + value.provinceId + '">' + value
                    .provinceName + '</option>');
            });
    ...
    $.post(base_url + "service/Locationforregister/getDistrict", {
        ...
        function(data) {
            ...
            $.each(district, function(index, value) {
                districtDropdownGarage.append('<option value="' + value.districtId + '">' + value
                    .districtName + '</option>');
            });
    ...
    $.post(base_url + "service/Locationforregister/getSubdistrict", {
        ...
        function(data) {
            ...
            $.each(subDistrict, function(index, value) {
                subdistrictDropdownGarage.append('<option value="' + value.subdistrictId + '">' + value
