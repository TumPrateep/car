...
var table = $('#user-table').DataTable({
    ...
    "columnDefs": [{
        ...
        // },
        {
            ...
            "render": function(data, type, full, meta) {
                if (data.category == null) {
                    return '<a href="' + base_url + "admin/Usermanagement/editrole/" + data.id +
                        '"><button type="button" class="btn btn-dark"><i class="fa fa-user-plus" aria-hidden="true"></i></button></a> ' +
                        '<a href="' + base_url + "admin/Usermanagement/updateUser/" + data.id +
                        '"><button type="button" class="btn btn-warning"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a> ' +
                        '<button type="button" class="delete btn btn-danger" onclick="deleteUser(' +
                        data.id + ',\'' + data.username + '\')"><i class="fa fa-trash"></i></button>';
                }
                ...
                return '<a href="' + base_url + "admin/usermanagement/view/" + data.id +
                    '"><button type="button" class="btn btn-info"><i class="fa fa-eye" aria-hidden="true"></i></button></a> ';
                // '<a href="' + base_url + "admin/Usermanagement/editrole/" + data.id +
                // '"><button type="button" class="btn btn-dark"><i class="fa fa-user-plus" aria-hidden="true"></i></button></a> ' +
                // '<a href="' + base_url + "admin/Usermanagement/updateUser/" + data.id +
                // '"><button type="button" class="btn btn-warning"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a> ';
                // '<button type="button" class="delete btn btn-danger" onclick="deleteUser(' + data
                // .id + ',\'' + data.username + '\')"><i class="fa fa-trash"></i></button>';
            }
        }, {
            "targets": 3,
            "data": null,
            "render": function(data, type, full, meta) {
                ...
                return '<div>' +
                    ...
                    '" autocomplete="Off" onclick="updateStatus(' + data.id + ',' + data.status +
                    ')">' +
